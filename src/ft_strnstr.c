...
static void	ft_skip_table(size_t	list_ascii[256],
		char	*neddle, const size_t	n_len)
{
	size_t			i;
	...
	while (i < n_len - 1)
	{
		list_ascii[(size_t)neddle[i]] = ft_max((n_len - i - 1), 1);
		i++;
	}
}
...
// 	size_t			i;
// 	const size_t	n_len = ft_strlen(neddle);
...
// 	while (i <= h_len - n_len)
// 	{
// 		if (haystack[i + n_len - 1] == neddle[n_len - 1])
// 			if (ft_memcmp(haystack + i, neddle, n_len - 1) == 0)
// 				return (haystack + i);
...
char	*ft_strnstr(const char *haystack, const char *neddle, size_t n)
{
	...
	if (neddle == NULL || n_len == 0)
		return (haystack);
	if (n_len > n || !haystack)
		return (NULL);
	ft_skip_table(list_ascii, neddle, n_len);
	i = 0;
	while (n-- > 0 && i < (h_len - n_len))
	{
		if (haystack[i + n_len - 1] == neddle[n_len - 1])
			if (ft_memcmp(haystack + i, neddle, n_len - 1) == 0)
				return (haystack + i);
